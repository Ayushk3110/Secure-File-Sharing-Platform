...
class File(db.Model):
    ...
    stored_filename = db.Column(db.String(255), unique=True, nullable=False)  # Unique filename on disk
    encrypted_file_key = db.Column(db.String(255), nullable=False)  # File-specific key, encrypted by master key
...
class ShareLink(db.Model):
    ...
    access_token = db.Column(db.String(64), unique=True, nullable=False)  # UUID for the link
    expires_at = db.Column(db.DateTime, nullable=True)  # Null for no expiration
    max_downloads = db.Column(db.Integer, nullable=True)  # Null for unlimited downloads
    ...
    for f in user_files:
        ...
        for link in links:
            if link.expires_at:
                ...
                aware_expires_at = link.expires_at.replace(tzinfo=timezone.utc) if link.expires_at.tzinfo is None else link.expires_at
...
def generate_link(file_id):
    ...
    if request.method == "POST":
        ...
        shareable_url = url_for("download_file_via_link", token=access_token, _external=True)
...
def download_file_via_link(token):
    ...
    if share_link.expires_at:
        ...
        aware_expires_at = share_link.expires_at.replace(tzinfo=timezone.utc) if share_link.expires_at.tzinfo is None else share_link.expires_at
        if aware_expires_at < now_utc:
    ...
    # Check max downloads
    if share_link.max_downloads is not None and share_link.current_downloads >= share_link.max_downloads:
    ...
    try:
        ...
        decrypted_data = decrypt_file(encrypted_data, file_record.encrypted_file_key.encode())

        if decrypted_data is None:
            flash("Failed to decrypt file. It might be corrupted or the key is invalid.", "danger")
            abort(500)
        ...
        temp_decrypted_file_path = os.path.join(app.config['UPLOAD_FOLDER'], f"temp_decrypted_{uuid.uuid4()}_{file_record.original_filename}")
